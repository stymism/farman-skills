...
- **ZIPの生成（動作中のPCで）:** このスキルフォルダ内の `make-newpc-zip.ps1` を実行 → `ドキュメント\plaud-newpc-setup.zip`（約5KB）が出力される。中身は `SETUP.cmd` / `setup-newpc.ps1` / `README.txt` / **`plaud-config.json`（鍵）**。トークンを更新したら作り直す（ユーザーが「新PC用ZIP作って」と言ったらこれを実行する）
