...
| 環境 | 導入方法 |
|------|---------|
| **新しいPC / 未移行PC** | PowerShellに1行貼るだけ: `irm https://raw.githubusercontent.com/stymism/farman-skills/main/migrate.ps1 \| iex` → Claude Code再起動。（旧ジャンクション撤去→マーケットプレイス登録→3プラグイン導入→設定ファイル有無チェック。冪等・再実行安全） |
...
### 🔧 初回セットアップ（設定ファイルが無いPCのみ・1回）

スキル本体は上記の1行コマンドで入る。**設定ファイル `~/.plaud/plaud-config.json` だけはローカル生成が必要**（トークン入りのためgit配布しない）。

- **推奨:** 動作中のPCから `~/.plaud/plaud-config.json` をコピーして持ち込む（USBや安全な手段で。作業ディレクトリのパスが異なる場合は `paths.work_dir` を編集）
- **または対話生成:** このスキルフォルダ内の `setup-plaud.ps1` を実行。前提条件チェック→Plaud RESTトークン・Cloudflareトークン入力→作業ディレクトリ指定→ `~/.plaud/plaud-config.json` を生成（UTF-8 BOM付き）。**AsanaはMCPコネクタ経由のためトークン不要**

**設定ファイルの保護:** トークン・パスワードを含むため**git管理は禁止**（スキルrepo・サイトrepoとも `.gitignore` で除外済み）。持ち運びはUSB等の手渡しで（メール・チャット・GitHub経由は禁止）。

**デプロイ（STEP7）もするPCは、さらに2つ:**

1. **git + GitHub CLI の導入と認証**（1回だけ）:
   ```powershell
   ...
   ```
2. **作業ディレクトリをサイトrepoからseedする**（重要）。新PCの作業ディレクトリは空であり、空のままSTEP7を実行すると公開サイトを空内容で上書きする事故につながる（STEP7に自動ガードあり＝空に近いwork_dirからのデプロイは中止される）。seed手順:
   ```powershell
   ...
   git clone https://github.com/stymism/farman-mtg-site.git "$env:USERPROFILE\.plaud\farman-mtg-site"
   New-Item -ItemType Directory -Force $wd | Out-Null
   robocopy "$env:USERPROFILE\.plaud\farman-mtg-site" $wd /E /XD .git /XF .gitignore SITE-REPO-README.md /NFL /NDL /NJH /NJS /NP
   ```
