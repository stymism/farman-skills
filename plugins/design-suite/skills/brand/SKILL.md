...
5. **commit → push** — `git -C "C:\claude code\skills-marketplace" add -A && git commit -m "brand: <要点>" && git push origin main`（**push前にユーザーへ確認**）。完了報告に「各PCは `claude plugin update design-suite@farman-skills` で取り込む」と明記する。
