...
**ローカル Claude Code / Cowork のどちらでも動く。**

| | ローカル | Cowork |
|---|---|---|
| ファイルの渡し方 | 設定ファイルにパスを書く | 4ファイルをアップロード |
| 出力 | `outputDir` に保存 | 出力ディレクトリ→ダウンロード |
| Gitのインストール | 不要 | **不要**（プラグイン取得はサーバー側で行われる） |

Coworkでは `--daily-log` `--ledger` `--template` `--out-dir` でパスを上書きする。
`openpyxl` が無ければ `pip install openpyxl`。

## 変更履歴

- 2026-08-07 作成日(madeOn)を省略可にした。固定のままだと翌年も同じ日付が入ってしまうため。
  設定にパスが無くても引数だけで動くようにし、不足時は分かるエラーを出す。
- 2026-08-07 Cowork対応。パスをコマンド引数で上書きできるようにし、
  アップロードされたファイルだけで完結できるようにした。
