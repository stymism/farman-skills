...
(function () {
  ...
  window.addEventListener('message', function (e) {
    ...
    var d = e.data || {};
    if (typeof d.farmanEstimateHeight === 'number' && d.farmanEstimateHeight > 300 && d.farmanEstimateHeight < 20000) {
      f.style.height = (d.farmanEstimateHeight + 4) + 'px';
    }
    if (typeof d.farmanEstimateScrollTo === 'number') {
      var y = f.getBoundingClientRect().top + window.pageYOffset + d.farmanEstimateScrollTo;
...
## スクロールの親ページ委譲（実装済み・高さ自動調整とセット）

高さ自動調整でiframeが中身ぴったりになると、**iframe内部にスクロールの余地が無くなり
`window.scrollTo` が効かなくなる**。その結果、品目カードを選んでも規格ピッカーまで自動移動せず、
小計や「次へ」が画面外のままで「押しても何も起きない」ように見える不具合になった。

対策として `index.html` の `scrollAppTo(y)` が、埋め込み時（`window.top !== window.self`）は
`postMessage({ farmanEstimateScrollTo: y })` を親へ送り、親のリスナー（上記スニペット）が
`iframeの位置 + y` へ実際にスクロールする。`y` はアプリ先頭からの相対位置。

改修時の要点:

- **画面内スクロールを伴う処理は `window.scrollTo` を直接呼ばず `scrollAppTo()` を使う。**
  埋め込み時は親、非埋め込み時は自分をスクロールする両対応になっている
- 現状 `scrollAppTo` の呼び出しは4か所（`showInlineError`＝先頭のエラーバナー表示、
  `showScreen`／`showStep`＝画面切替で先頭へ、`openVariantPicker`＝規格ピッカーへ）
- クロスオリジン（GASサンドボックス）のため `element.scrollIntoView()` では親を動かせない。
  親スクロールは postMessage しか手段がない
- 親がリスナーを持たない場合はスクロールが効かないだけ（従来動作）で壊れはしない
